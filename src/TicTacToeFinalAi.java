...
public class TicTacToeFinalAi {
    ...
    /**
     * objet random de la classe Random pour que l'ia choisisse aléatoire 
     * une colonne et une ligne pour la première fois
     */
            ...
            if (menu == 1) {
                ...
                if (currentPlayer == playerX) {
                    ...
                    if (isValidMove(row, col)) {
                        ...
                        checkWinner();
                        if (!gameOver) {
                            currentPlayer = playerO; // Passage à l'IA (joueur O)
                        }
                    } 
                    else {
                        System.out.println("Coup invalide. Réessayez.");
                    ...
                    checkWinner();
                    if (!gameOver) {
                        currentPlayer = playerX; // Retour au joueur humain
                    }
                }
                printBoard();
            }
    ...
    void playAIMove(){
        if (turns > 0){
            ...
            if (isValidMove(row, col)) {
                ...
                checkWinner();
                if (!gameOver) {
                    currentPlayer = playerX; // Passage à l'humain (joueur x)
                }
            }
        }
        else{
